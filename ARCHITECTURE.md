...
3. Next.js verifies the logged-in Clerk user on the server.
4. Next.js creates or finds the KeepDB user row by email and Clerk user id.
...
- Clerk publishable key: public browser key for login UI.
- Clerk secret key: server-only key used by Clerk's Next.js integration.
...
- `src/lib/auth/current-user.ts`: requires a signed-in Clerk user.
...
- `src/lib/keepdb/keep-user.ts`: maps the Clerk user to a KeepDB user row.
